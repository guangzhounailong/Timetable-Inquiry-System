...
private:
    ...
    wxWindow* BuildRawPage(wxWindow* parent) {
        ...
        sizer->Add(rawResponseCtrl_, 1, wxEXPAND | wxALL, 12);
    ...
    void SaveLoadedCourseChangesAsync() {
        ...
        worker_ = std::thread([this, courseCode, section, changes]() {
            ServerResponse finalResponse;
            finalResponse.ok = true;
            finalResponse.connected = client_.isConnected();

            for (const auto& change : changes) {
                ServerResponse response = client_.sendCommand("UPDATE " + courseCode + " " +
    ...
    void OnCommandFinished(wxThreadEvent& event) {
        ...
        if (!response.ok) {
            AppendRawLog(command, response.error);
            SetStatusText(stdToWx(response.error));
            return;
        }

        AppendRawLog(command, response.rawText());
    ...
    void AppendRawLog(const std::string& command, const std::string& text) {
        if (rawResponseCtrl_ == nullptr) {
            return;
        }

        if (!rawResponseCtrl_->IsEmpty()) {
            rawResponseCtrl_->AppendText("\n");
        }
        rawResponseCtrl_->AppendText(stdToWx("> " + command + "\n" + text));
